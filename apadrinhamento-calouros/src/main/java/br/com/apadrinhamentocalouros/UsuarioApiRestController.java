...
public class UsuarioApiRestController {
	...
	//
	// @RequestMapping(value = "lance/{idLeilao}", method = RequestMethod.GET,
	// produces = "application/json")
	// public List<Lance> getLances(@PathVariable Long idLeilao) {
	// Iterable<Lance> lances = lanceService.findByIdLeilao(new
	// Leilao().builder().idLeilao(idLeilao).build());
	// List<Lance> retorno = new ArrayList<Lance>();
	// for (Lance lance : lances) {
	// retorno.add(new
	// Lance().builder().valorString(lance.getValorString()).dataHora(lance.getDataHora()).build());
	// }
	//
	// return retorno;
	// }
	//
	// @RequestMapping(value = "leilao/informacoes/{idLeilao}", method =
	...
	// .valorString(leilao.isFinalizado() || leilao.getTipoLance().idTipoLance == 2
	// ? leilao.getValorString() : "")
	// .build();
	// }
}
